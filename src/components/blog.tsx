          ...
          <div className="flex flex-wrap items-center justify-center gap-x-14 gap-y-10 md:gap-y-12 lg:gap-y-12 xl:gap-y-16 w-full">
            {
              product.map((data)=>{
                return(
                  <div key={data.id}>
                    <Image src={data.img_url} alt={data.title} width={350} height={350} className="w-60 h-60 md:w-64 md:h-64 lg:w-72 lg:h-72 xl:w-80 xl-h-80"></Image>
                    <p className="text-xs md:text-sm lg:text-base text-gray-500 pt-2">{data.date}</p>
                    <p className="text-base md:text-lg lg:text-xl font-semibold text-gray-500 pt-2  w-60 md:w-64 lg:w-72"><Link href={"/blogdetail1"}>{data.title}</Link></p>
                    <p className="text-xs md:text-sm lg:text-base text-gray-500 w-52 md:w-60 lg:w-72  text-justify pt-2"> {data.description}</p>
                  </div>
                )
              }
            )
